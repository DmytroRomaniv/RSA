namespace MyMD5
{
   public class MD5Algorithm
   
    {
        ...
        protected void FullByteInput(string data)
        {
            _byteInput = new byte[data.Length];

            for (int i = 0; i < data.Length; i++)
            {
                _byteInput[i] = (byte)data[i];
            }
        }

        protected void FullByteInput(byte [] data)
        {
            _byteInput = new byte[data.Length];
